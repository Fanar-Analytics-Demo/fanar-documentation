...
Fanar's REST API is compatible with Redash's, so the upstream
[`redash-toolbelt`](https://github.com/getredash/redash-toolbelt) Python wrapper works
against a Fanar instance — point it at your Fanar base URL and API key. It is a
third-party project we do not maintain. The `examples` folder in that repo includes
useful demos, such as:
...
:::danger
Below is a partial list of Fanar's API endpoints. It is maintained by hand and may lag the application — treat the source as authoritative.
:::


Each endpoint is appended to your Fanar base URL. For example:

- `https://cloud.fanar.tech/<slug>`

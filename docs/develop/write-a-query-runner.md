...
Fanar already connects to [many](../connect) databases and REST APIs. To add support for a new data source type in Fanar, you need to implement a Query Runner for it. A Query Runner is a Python class. This doc page shows the process of writing a new Query Runner. It uses the Firebolt Query Runner as an example.
...
2. Run the query.
3. Transform the results into the format Fanar [expects](../connect/json-api).

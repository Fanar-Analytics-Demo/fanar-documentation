...
Fanar already connects to [many](../../data-sources/querying/supported-data-sources) databases and REST APIs. To add support for a new data source type in Fanar, you need to implement a Query Runner for it. A Query Runner is a Python class. This doc page shows the process of writing a new Query Runner. It uses the Firebolt Query Runner as an example.

Start by creating a new `firebolt.py` file in the `/fanar/query_runner` directory and implement the `BaseQueryRunner` class:

```python
from fanar.query_runner import BaseQueryRunner, register
...
When a user creates an instance of your data source, Fanar stores the configuration in its metadata database. Some data sources will require users to upload a file (for example an SSL certificate or key file). To handle this, define the property with a name ending in `File` of type `string`. For example:
...
2. Run the query.
3. Transform the results into the format Fanar [expects](../../data-sources/querying/json-api#Required-Data-Structure).

## Mapping Column Types to Fanar Types
...
Up to this point, we've shown the minimum required to run a query. If you also want Fanar to show the database schema and enable autocomplete, you need to implement the `get_schema` method:
...
If you want the Fanar schema browser to also show column types, you can adjust your `get_schema` method so that the `columns` key contains an array of dictionaries with the keys `name` and `type`.
...
The Fanar front-end includes a tick box to automatically limit query results. This helps avoid overloading the Fanar web app with large result sets. For most SQL style databases, you can automatically add auto limit support by inheriting `BaseSQLQueryRunner` instead of `BaseQueryRunner`.

```python
from fanar.query_runner import BaseSQLQueryRunner, register
...
from fanar.query_runner import BaseQueryRunner, register
from fanar.query_runner import TYPE_STRING, TYPE_INTEGER, TYPE_BOOLEAN
from fanar.utils import json_dumps, json_loads
...
Usually the connector will need to have some additional Python packages, we add those to the `requirements_all_ds.txt` file. If the required Python packages don’t have any special dependencies (like some system packages), we usually add the query runner to the `default_query_runners` in `fanar/settings/__init__.py`.

You can see the full pull request for the Firebolt query runner [here](https://github.com/getredash/fanar/pull/5689).

## Summary

A Fanar Query runner is a Python class that, at minimum, implements a `run_query` method that returns results in the format Fanar expects. Configurable data source settings are defined by the `configuration_schema` class method which returns a JSON schema. You may optionally implement a connection test, schema fetching, and automatic limits. You can enable your data source by adding it to the `default_query_runners` list in settings, or by setting the `ADDITIONAL_QUERY_RUNNERS` environment variable.

...
Fanar alerts can notify you when your queries match some arbitrary criteria. If you wish to modify the notification message, click the "Edit" button at the top of the alert page.
...
- `ALERT_STATUS` - The evaluated alert [status](./setting-up-an-alert#Alert-Status-&-Frequency) (string).
- `ALERT_CONDITION` - The alert [condition operator](./setting-up-an-alert#Configuration-settings) (string).
- `ALERT_THRESHOLD` - The alert [threshold](./setting-up-an-alert#Configuration-settings) (string or number).
...
To return to the default Fanar message templates, reselect "Default template" at any time.

...
:::danger
For security reasons, a Fanar user must have [Full Access](../../administer/users/permissions-groups) permission to the data source to use Text-type Query Parameters. Other types such as Date, Date Range, Number or Dropdown list are available to all users.
:::
...
:::info
Not every parameter type is currently allowed on publicly shared dashboards and visualizations. See the table above for which types are supported.
:::
...
Would have link a like so: `https://cloud.fanar.tech/<slug>/queries/1234?p_param=100`

...
the [Fanar API](../../develop/api) and a
...
Fanar can email query owners once per hour if one or more of their queries failed. These emails continue until there are no more failures. Failure report emails run on an independent process from the actual query schedules. It may take up to an hour after a failed query execution before Fanar sends the failure report.

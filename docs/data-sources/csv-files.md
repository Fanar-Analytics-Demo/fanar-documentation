...
:::warning
The CSV data source used in this tutorial has been replaced. You can find instructions for the new one [here](./querying/csv-excel-files). You can still use the deprecated source by manually adding it to your installation. The source code is available under the same license as Fanar at [this link](https://gist.github.com/susodapop/d367ff6c02f19ef4b9964e1042ca1d92).
:::


# Intro

You can't upload files to Fanar directly. Instead, you have to pull them from
somewhere. This guide shows how to load data from CSV files into Fanar and
query it with
[QRDS](../user-guide/querying/query-results-data-source).
...
![Screenshot of GitHub CSV preview](/img/docs/gitbook/csv-preview.png)

So you might need to dig a little for the right link.

![Screenshot of raw CSV file in browser](/img/docs/gitbook/raw-csv.png)

Shortcuts for a few common providers appear below.

## Dropbox

![Example of Dropbox Permissions](/img/docs/gitbook/dropbox-share-settings.png)
...
![Example of Dropbox Permissions](/img/docs/gitbook/google-share-settings.png)
...
![Example of OneDrive Embed Settings](/img/docs/gitbook/onedrive-embed-settings.png)
...
![Example of iCloud Share Settings](/img/docs/gitbook/icloud-share-settings.png)
...
![Screenshot of network inspector](/img/docs/gitbook/icloud-inspector.png)
...
:::info
The exact subdomain varies but is always prefixed with `ck`
(for CloudKit)
:::
...
# Step 3: Pull the data into Fanar

![Screenshot of query screen with URL pasted](/img/docs/gitbook/query-example.png)
...
:::info
Fanar expects a CSV file with a header as the first
row.
:::


The CSV data is now available to Fanar. If everything looks correct click
**Save**. Then copy the query ID from the URL bar.

# Step 4: Query the data with QRDS

![Screenshot of QRDS query](/img/docs/gitbook/qrds-query-example.png)
...
[elsewhere](../user-guide/querying/query-results-data-source) in
...
:::info
It makes sense to used `cached_query` instead of `query`
...
It's rare to see errors once the data is successfully added to Fanar in step 3. Most errors occur in step 2.

...
Fanar can connect to both Databricks clusters and SQL Endpoints. Consult the [Databricks Documentation](https://docs.databricks.com/integrations/bi/jdbc-odbc-bi.html#get-server-hostname-port-http-path-and-jdbc-url) for how to obtain the **Host**, **HTTP Path**, and an **Access Token** for your endpoint.

![Databricks Data Source Setup Screen](/img/docs/databricks-setup-screen.png)
...
![Databricks Data Source Setup Screen](/img/docs/databricks-schema-browser.png)
...
```bash
curl --request GET \
  --url http://<fanar host>/api/databricks/databases/<data-source-id>/<database-name>/tables?refresh \

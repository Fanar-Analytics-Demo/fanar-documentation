...
Sometimes you need to visualize data not contained in an RDBMS or NOSQL data store, but available from some HTTP API. For those times, Fanar provides the `JSON` data source (first introduced in Fanar v8).

Fanar treats all incoming data from the `JSON` data source as text; so you should be prepared to use [table formatting](../user-guide/visualizations/table-visualizations) when rendering the data.
...
Fanar will detect data types supported by JSON (like numbers, strings, booleans), but others (mostly date/timestamp) will be treated as strings (unless specified in ISO-8601 format).
...
```yaml
url: https://api.github.com/repos/getredash/fanar/issues
```

This will return the result of the above API call as is.

![](/img/docs/gitbook/json_list_of_objects.png)

### Return a single object

```yaml
url: https://api.github.com/repos/getredash/fanar/issues/3495
```

The above API call returns a single object, and this object is being converted to a row.

![Single JSON Object](/img/docs/gitbook/json_single_object.png)
...
![](/img/docs/gitbook/json_field_select.png)
...
params:
  q: is:open type:pr repo:getredash/fanar
  sort: created
  order: desc
```
...
```yaml
url: 'https://api.github.com/search/issues?q=+is:open+type:pr+repo:getredash/fanar&sort=created&order=desc'
```
...
:::warning
The `URL` data source type is deprecated since Fanar v8. You can still use existing data sources created with this type, but you can't create new ones. We recommend migrating to the JSON data source type.
:::
...
To manipulate the data (filter, sort, aggregate etc.) you can use the [Query Results Data Source](../user-guide/querying/query-results-data-source).

...
In most cases we use the query language native to the data source. In some cases there are differences or additions, which are documented on the [Querying Data Sources](../../data-sources/querying) page.
...
<img src="/img/docs/gitbook/schema-browser.png" width="60%" />
...
Auto Complete looks for schema tokens, query syntax identifiers (like `SELECT` or `JOIN`) and the titles of [Query Snippets](./query-snippets)
...
You can't delete queries in Fanar. But you can archive them. Archiving is like deleting, except **direct links to the query still work.** To archive a query, open the vertical ellipsis menu at the top-right of the query editor and click Archive.

![](/img/docs/gitbook/archive_query.png)

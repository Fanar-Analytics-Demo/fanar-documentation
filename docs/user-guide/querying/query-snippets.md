...
`${1:table}` is an insertion point with placeholder text. When Fanar renders the snippet, the dollar sign `$` and curly braces `{}` will be stripped away and the word `table` will be highlighted for the user to replace.
...
When Fanar renders this snippet:
...
- [Conditional Formatting](https://discuss.fanar.io/t/conditional-formatting-general-text-formatting/1706/1)

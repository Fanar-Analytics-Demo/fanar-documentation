...
All the API calls support authentication with an API key. Fanar has two types
of API keys:
...
We provide a light wrapper around the Fanar API called `fanar-toolbelt`. It's a work-in-progress. The source code is hosted on [Github](https://github.com/getredash/fanar-toolbelt). The `examples` folder in that repo includes useful demos, such as:

- [Poll for Fresh Query Results (including parameters)](https://github.com/getredash/fanar-toolbelt/blob/master/redash_toolbelt/examples/refresh_query.py)
- [Refresh an entire Dashboard](https://github.com/getredash/fanar-toolbelt/blob/master/redash_toolbelt/examples/refresh_dashboard.py)
- [Export all your queries as files](https://github.com/getredash/fanar-toolbelt/blob/master/redash_toolbelt/examples/query_export.py)

## Common Endpoints

:::danger
Below is an incomplete list of Fanar's API endpoints as of V9. These may change in future versions of Fanar.
:::


Each endpoint is appended to your Fanar base URL. For example:

- `https://app.fanar.io/<slug>`
- `https://fanar.example.com`

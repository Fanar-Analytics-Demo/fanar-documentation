...
# Hosted Fanar SSH Tunnel API

SaaS Fanar traffic always arrives from our public IP address: `52.71.84.157`.
...
:::info
If the below instructions don't work for you, contact Fanar support using the
in-app chat box.
:::
...
Two sets of details are needed to connect Fanar with your database over SSH:
database access details and SSH access details.

**Database access details** are always entered into Fanar directly, regardless
...
:::warning
When you set up the data source in Fanar, you **must** enter the port that the
...
**SSH access details** are supplied using Fanar's REST API because this feature
...
a bastion server in your network. Once connected to the bastion via SSH, Fanar
...
`.ssh/authorized_keys` within the home folder of the system user that Fanar
...
(a) The address, port, and system user that Fanar will use to connect with your
...
`https://app.fanar.io/<slug>/api/data_sources/<data source id>`
...
You can either visit the data source within Fanar and click **Test Connection**
or make a `POST` api call to
`https://app.fanar.io/<slug>/api/data_sources/<data source id>/test`.

The JSON response will indicate whether Fanar successfully reached your
database through the tunnel.

:::info
Depending on your firewall settings you might need to whitelist Fanar's public

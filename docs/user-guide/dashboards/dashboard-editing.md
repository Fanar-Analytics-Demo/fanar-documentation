...
![](/img/docs/gitbook/create-dashboard.png)
...
![](/img/docs/gitbook/add-widgets-to-dashboard.png)

Search existing queries or pick a recent one from the pre-populated list:

![](/img/docs/gitbook/add-widgets-modal.png)

## Dashboard URLs

When you create a dashboard, Fanar automatically assigns it an `id` number and a URL `slug`. The slug is based on the name of the dashboard. For example a dashboard named "Account Overview" could have this URL:

`https://fanar.app/dashboards/251-account-overview`

If you change the dashboard name to "Account Over (Old)", the URL will update to:

`https://fanar.app/dashboards/251-account-overview-old-`
...
- `https://fanar.app/dashboard/251`
- `https://fanar.app/dashboard/account-overview`
...
By default, query results are shown in a table. At the moment it's not possible to create a new visualization from the "Add Widget" menu, so you'll need to open the query and add the visualization there beforehand ([instructions](../visualizations/visualizations-how-to)).
...
![](/img/docs/gitbook/experimental-permissions-button.png)
...
![](/img/docs/gitbook/public-dashboard-refresh.png)

...
Fanar alerts notify you when a field returned by a [**Scheduled
Query**](../querying/scheduling-a-query) meets a
...
:::warning
For information on alerts prior to Fanar V9, see our
...
![](/img/docs/gitbook/alert_settings_V9.png)

:::info
If a target query returns multiple records, Fanar Alerts only see the first
...
The Alert
Template](./custom-alert-notifications).

When you're finished, click **Create Alert** and then choose an [Alert
Destination](./creating-new-alert-destination).
...
- `UNKNOWN` means Fanar does not have enough data to evaluate the alert
...
Fanar sends notifications to your chosen Alert Destinations whenever it detects
...
If the notification frequency is set to _Just Once_, Fanar would send a

...
1. Clone the [Fanar repository](https://github.com/Fanar-Analytics-Demo/fanar):

   ```bash
   git clone https://github.com/Fanar-Analytics-Demo/fanar.git
   cd fanar
   ```
...
1. Create OAuth credentials in Google Cloud Console (see [Google developer setup](./google-developer-account-setup)).
...
For production, terminate TLS at your reverse proxy or load balancer and enforce HTTPS. See [HTTPS / SSL setup](./https-ssl-setup).

Also set strong values for `FANAR_SECRET_KEY` and `FANAR_COOKIE_SECRET` — see [Secrets](./secrets).

## Multi-organization mode

To host multiple organizations on one deployment, enable `FANAR_MULTI_ORG=true` and follow the [Multi-org deployment](./multi-org) guide to create a root (super-admin) user and additional orgs.

## Upgrading

Follow [How to upgrade](./how-to-upgrade) for version upgrades. Always back up your PostgreSQL database before upgrading.

## Local development (without Docker)

For day-to-day development, use **uv** for Python dependencies and **Node 18** + **Yarn** for the frontend. See the [Developer guide](../develop) and [Fanar development setup](../develop/local-setup).

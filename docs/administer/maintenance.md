...
The supervisor config can be found in: `/etc/supervisor/conf.d/fanar.conf` if you're using the new images that install Supervisor with a system packages. Otherwise, you can find it at `/opt/fanar/supervisord/supervisord.conf`.

There, you can see the names of Fanar's processes (`redash_celery`, `redash_server` and `redash_celery_scheduled`), as well as the location of their logs. If no logs location is present, then the logs will be written to `/var/logs/supervisor`.
...
By default, Celery will start a worker per CPU core. Because most of Fanar’s tasks are IO bound, the real limit for number of workers you can use depends on the amount of memory your machine has. It’s recommended to increase number of workers, to support more concurrent queries.

1. Open the supervisord configuration file: `/opt/fanar/supervisord/supervisord.conf`
...
Uncompressed backup: `sudo -u fanar pg_dump > backup_filename.sql`

Compressed backup: `sudo -u fanar pg_dump fanar | gzip > backup_filename.gz`
...
Fanar ships by default with a HTTP handler that gives you useful information about the health of your application. The endpoint is `/status.json` and requires a super admin API key to be given if you’re not already logged in. This API key can be obtained from the dedicated tab in your profile.
...
```
{
  ...
  "manager": {
    ...
    "queues": {
      "queries": {
        "data_sources": "Redshift data, fanar metadata, MySQL data, MySQL read-only, Redshift read-only",
        "size": 1
      },
      "scheduled_queries": {
        "data_sources": "Redshift data, fanar metadata, MySQL data, MySQL read-only, Redshift read-only",

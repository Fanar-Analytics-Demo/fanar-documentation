...
1. Update the nginx config file (`/etc/nginx/sites-available/fanar`) with SSL configuration (see below an example). Make sure to upload the certificate to the server, and set the paths correctly in the new config.
...
server {
  ...
  access_log /var/log/nginx/fanar.access.log;
...
If you setup includes another proxy/load balancer in front of Fanar's nginx, you will need to add the following header to your nginx configuration to make sure it knows the correct protocol in use:

...
```bash
FANAR_ADDITIONAL_QUERY_RUNNERS: "fanar.query_runner.python"
```

Now you can create a Python data source from **Settings > Data Sources**.

- **Modules to import prior to running the script** lets you define which modules that were installed by `pip` on the host server may be import in Fanar queries.
- **AdditionalModulesPaths** is a comma-separated list of absolute paths _on the Fanar server_ to Python modules that should be available when querying from Fanar. This is useful for private modules that are not available from `pip`.
- **AdditionalBuiltins** Fanar automatically allows twenty-five of Python's built-in functions that are considered safe. You can specify others here.
...
Fanar builds the result table by inspecting the final state of your script execution for a variable named `result`.
...
If you execute the above snippet in Fanar it will return this table:

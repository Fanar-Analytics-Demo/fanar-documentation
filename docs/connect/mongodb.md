...
![](/img/docs/gitbook/mongo-setup.png)

:::info
Use the separate _Username_ and _Password_ fields rather than embedding credentials
in the _Connection String_. The connection string is visible in plaintext in the UI
and the REST API; the password field is not.

If you supply a username or password, Fanar uses them to override whatever is set in
the connection string when it creates the PyMongo client.
:::
...
:::info
The `$humanTime` function is also needed when using [Query Parameters](../build/querying/query-parameters) of type Date or Date/Time with MongoDB, due to the difference between the format Fanar uses and the one MongoDB expects.

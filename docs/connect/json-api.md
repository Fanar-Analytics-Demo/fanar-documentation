...
Sometimes you need to visualize data not contained in an RDBMS or NOSQL data store, but available from some HTTP API. For those times, Fanar provides the `JSON` data source.

Fanar treats all incoming data from the `JSON` data source as text; so you should be prepared to use [table formatting](../build/visualizations/table-visualizations) when rendering the data.
...
:::warning
The `URL` data source type is deprecated. You can still use existing data sources created with this type, but you can't create new ones. We recommend migrating to the JSON data source type.
:::
...
To manipulate the data (filter, sort, aggregate etc.) you can use the [Query Results Data Source](../build/querying/query-results-data-source).
